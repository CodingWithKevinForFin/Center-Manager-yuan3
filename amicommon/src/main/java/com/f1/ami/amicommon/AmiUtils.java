...
public class AmiUtils {
	...
	public static AmiAuthenticatorPlugin loadAuthenticatorPlugin(ContainerTools tools, String propertyName, String description) {
		...
		try {
			...
			if (r instanceof AmiAuthenticatorDisabled || r instanceof AmiAuthenticatorFileBacked)
				return r;
			return new AmiAuthenticatorApplyEntitlements(tools, props, r);
		} catch (Exception e) {
	...
	public static ServerSocketEntitlements parseWhiteList(ContainerTools props, String property) {
		...
		if (value.startsWith("file:")) {
			...
			try {
				AmiServerSocketEntitlementsPlugin t = loadPlugin(plugin, "ServerSocketEntitlements", props, props, AmiServerSocketEntitlementsPlugin.class);
				t.init(props, props);
	...
	//			} else
	//				sb.append(',').append(' ');
	//			toJson(e.getKey() == null ? "<null>" : e.getKey().toString(), sb);
	//			sb.append(':');
	...
	//		return sb;
	//	}
	//	private static void toJson(Object value, StringBuilder sb) {
	//		if (value instanceof Number || value instanceof Boolean || value == null || value instanceof Map || value instanceof List)
	//			s(value, sb);
	//		else {
	//			sb.append(SH.CHAR_QUOTE);
	//			SH.toStringEncode(value.toString(), SH.CHAR_QUOTE, sb);
	//			sb.append(SH.CHAR_QUOTE);
	//		}
	//	}
	//	public static StringBuilder join(Collection<?> tokens, StringBuilder r) {
	...
	public static void findAndFixDsName(Object o, Map<String, Object> sink) {
		// rules:
		// 1. Skips comments
		// 2. Disregards syntax validity
		// 3. Disregards ds name validity
		if (o instanceof List) { // should always hit
			List<String> lines = (List<String>) o;
			fixUseDs(lines);
			sink.put("amiscript", lines);
		}
	}

	// fix in place
	public static boolean fixUseDs(List<String> lines) {
		...
		for (int j = 0; j < lines.size(); j++) {
			...
			for (int s = 0; s < statements.size(); s++) {
				...
				// use ds
				//    ^
				if (!scr.expectSequenceNoThrow("use ")) {
					continue;
				}
				scr.skip(ExpressionParserHelper.WS);
				if (!scr.expectSequenceNoThrow("ds")) {
					continue;
				}
				scr.skip(ExpressionParserHelper.WS);
				...
				sb2.append('"').append(sb.toString()).append('"').toString();
				statement = SH.splice(statement, scr.getCountRead(), sb.length(), sb2.toString());
				statements.set(s, statement);
				statementChanged = true;
			}
			// fixed all statements on this line
			if (statementChanged) {
		...
		// returns true if there was a change in the statement
		return changed;
	}
}
