...
public class AmiRelayHelper {
	...
	public static CalcFrame toValues(CalcTypes types, IntKeyMap<Tuple2<String, Caster<?>>> keys, byte[] data, String type, String id) {
		if (keys.isEmpty())
		...
		for (int i = 0, valPos = (keysLength << 1) + 2, len; i < keysLength; i++, valPos += len - 1) {
			...
			Object value;
			Tuple2<String, Caster<?>> tuple = keys.get(key);
			if (tuple != null) {
				try {
