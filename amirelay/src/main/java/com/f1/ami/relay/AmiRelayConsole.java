...
public class AmiRelayConsole {
	...
	public String showRoutes() {
		...
		BasicTable r = new BasicTable(String.class, "Name", String.class, "TargetCenters", Long.class, "MatchCount", Long.class, "MismatchCount");
		r.setTitle("Transforms(Order by Priority)");
		if (routes != null) {
			for (AmiRelayRoute t : routes) {
				long match = t.getMatchCount();
				long mismatch = t.getMismatchCount();
				r.getRows().addRow(t.getRouteName(), SH.join(",", t.getRouteList()), match, mismatch);
			}
		}
		r.toString(sb);
		return sb.toString();
