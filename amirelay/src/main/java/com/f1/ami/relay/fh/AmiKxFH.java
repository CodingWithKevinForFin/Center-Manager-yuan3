...
public class AmiKxFH extends AmiFHBase {
	...
	private static class TableEntry {
		...
		//		final private HashMap<String, Object> p;
		//		final private Entry<String, Object>[] entries;
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (c == null) {
						onFailed("Failed to connect to url: " + kxUrl);
						return;
					}

					log.info("connected to " + kxUrl);
					...
					LH.severe(log, "got an exception waiting for updates from kx: ", Arrays.toString(o), e);
	...
	protected void recover(Object params) {
		if (replayUrl != null) {
			...
			try {
				...
				log.info("Recovering of " + chunks + " chunks is complete");
	...
	protected void readSchema(Object meta) {
		...
		for (int i = 0; i < f.x.length; i++) {
			...
			if (this.debug == true) {
				sb.append("Reading schema for subscription to table `").append(tableName);
	...
	public c GetConnection(String url, String username, String password, AmiNamingServiceResolver serviceResolver) {
		try {
			...
			LH.log(log, Level.INFO, "Failed to connect to url: ", url, " message: " + e.getMessage());
			return null;
		} catch (Exception e) {
			LH.log(log, Level.SEVERE, "Failed to connect to url: ", url, e);
			return null;
		}
	}
