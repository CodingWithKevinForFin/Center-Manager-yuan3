...
public class AmiRelayRoutes {
	...
	public AmiRelayRoutes(ContainerTools tools, AmiRelayState state, long delay) {
		...
		this.threadSafeRouter = new AmiRelayRouter(fullPath, null, sf, false, null, debug);
	...
	private void parse(Cache data) {
		...
		try {
			...
			ReusableCalcFrameStack sf = new ReusableCalcFrameStack(state.createStackFrame());
			this.threadSafeRouter = new AmiRelayRouter(fullPath, AH.toArray(routesByName.values(), AmiRelayRoute.class), sf, hasRules, chain, debug);
		} finally {
