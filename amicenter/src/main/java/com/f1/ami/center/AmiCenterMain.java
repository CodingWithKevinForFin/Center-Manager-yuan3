...
public class AmiCenterMain {
	...
	public static DispatchController main2(ContainerBootstrap cb) throws Exception {
		...
		ServerSocketEntitlements entitlements = AmiUtils.parseWhiteList(container.getTools(), AmiCenterProperties.PROPERTY_AMI_CENTER_PORT_WHITELIST);
		...
		Factory fountain = new BatchLongSequenceIdGenerator.Factory(new FileBackedLongSequenceIdGenerator.Factory(fountainDir), 1000000);
		...
		if (jdbcPort > 0) {
			...
			final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(container.getTools(), AmiCenterProperties.PROPERTY_AMI_DB_JDBC_PORT_WHITELIST);
		...
		if (jdbcSslPort > 0) {
			...
			SimpleMessagingServer sms = new SimpleMessagingServer("AMI JDBC", jdbcSslPortBindAddr, sse, jdbcSslPort,
					new AmiCenterJdbcServer(jdbcaa, itineraryPort, amiDbDialects, jdbcVersion), container.getServices().getConverter(), jdbcExecutor, jdbcVersion, jdbcSslKeystore, jdbcSslKeystorePass);
		...
		if (amiConsolePort > 0) {
			...
			final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(container.getTools(), AmiCenterProperties.PROPERTY_AMI_DB_CONSOLE_PORT_WHITELIST);
