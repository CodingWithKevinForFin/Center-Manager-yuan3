		...
		try {
			...
			if (columns != null) {
				...
				for (int i = 0; i < columns.size(); i++) {
					Column col = targetTable.getColumnsMap().get(columns.get(i));
					if (col == null)
						throw new AmiDatasourceException(AmiDatasourceException.SCHEMA_ERROR, "Column not found in table '" + targetTableName + "': " + col);
