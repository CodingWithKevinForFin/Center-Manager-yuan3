...
public class AmiCenterConsoleCmd_Sql extends AmiCenterConsoleCmd {
	...
	public void process(AmiCenterConsoleClient client, String cmd, String[] cmdParts) {
			...
			for (Table i : action.getTables()) {
				...
				for (int n = 0; n < formatters.length; n++) {
					...
					if (type == DateMillis.class || type == DateNanos.class)
						f = client.getDatetimeFormat();
					else if (OH.isFloat(type))
						f = client.getDecimalFormat();
					else if (Number.class.isAssignableFrom(type))
						f = client.getNumberFormat();
					else
						f = null;
		...
		if (hasReturnValue) {
			...
			if (returnValue == null)
				...
				if ((returnType == DateMillis.class || returnType == DateNanos.class) && client.getDatetimeFormat() != null)
					out.println(client.getDatetimeFormat().format(returnValue));
				else {
		...
		StringBuilder sb = new StringBuilder();
		DateFormatNano f = client.getDatetimeFormat();
	...
	public void init(AmiCenterConsoleClient client) {
		...
		client.addLocalSetting("limit", "-1", Integer.class);
