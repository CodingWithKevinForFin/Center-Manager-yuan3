		...
		if (useOptions != null) {
			...
			for (Entry<String, Node> s : useOptions.entrySet()) {
				...
				if (value == null)
					...
					if (t == null || t <= 9L)
						throw new ExpressionParserException(s.getValue().getPosition(), "Invalid value, must be number greater than 9: " + value);
					refreshPeriodMs = t;
	...
	public TableReturn processRowAdds(CalcFrameStack sf, Table table, int tableNamePos, int positions[], Object[][] rows, boolean returnGeneratedIds) {
		...
		if (returnGeneratedIds) {
			...
			for (Object[] row : rows) {
				...
				for (int i : positions)
					pr.putAt(i, row[i]);
				AmiRowImpl row2 = t.insertAmiRow(pr, true, sf);
				if (row2 != null) {
					if (pi != null)
	...
	public Table processShow(CalcFrameStack sf, String targetType, int targetTypePos, int scope, boolean isFull, String name, int namePos, String from, int fromPos,
		...
		if ("TABLES".equalsIgnoreCase(targetType)) {
			...
			for (String varname : consts.getVarKeys()) {
				String value = OH.toString(consts.getValue(varname));
				if (!isFull)
					value = SH.ddd(value, 80);
				r.getRows().addRow(varname, mf.forType(consts.getType(varname)), value, true);
			}
			CalcFrame vars = session.getVars();
			for (String varname : vars.getVarKeys()) {
				String value = OH.toString(vars.getValue(varname));
				if (!isFull)
					value = SH.ddd(value, 80);
				r.getRows().addRow(varname, mf.forType(vars.getType(varname)), value, false);
			}
			return r;
			...
			return r;
		} else if ("TABLE".equalsIgnoreCase(targetType) || "TABLES".equalsIgnoreCase(targetType) || "COLUMNS".equalsIgnoreCase(targetType)
				|| "METHODS".equalsIgnoreCase(targetType))
