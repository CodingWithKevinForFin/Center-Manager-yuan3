...
public class AmiWebBalancerServer implements Runnable {
	...
	public AmiWebBalancerServer(ScheduledThreadPoolExecutor threadPoolExecutor, CachedFile serversFileCache, File sessionsFileCache, long checkFilesFrequency,
			long keepAliveCheckMillis, long sessionTimeoutMs, String testUrl, long testPeriod) throws Exception {
		this.testHttpMethod = ("GET " + testUrl + " \n\n").getBytes();
					...
					if (parts.length >= 3) {
						...
						if (SH.is(ruleStr) && SH.is(targetsStr)) {
							if (SH.isnt(followupAction) || ON_FAILURE_CONTINUE.equalsIgnoreCase(followupAction) || ON_FAILURE_BREAK.equalsIgnoreCase(followupAction)) {
								AmiWebBalancerRouteRule rule = new AmiWebBalancerRouteRule(ruleStr, targetsStr, ON_FAILURE_CONTINUE.equalsIgnoreCase(followupAction));
								rules.add(rule);
		...
		synchronized (this) {
			...
			if (toRemove != null) {
				for (String i : toRemove) {
					...
					client.getConnectionStats(this.closedClientsConnectionsCount, this.closedClientsBytesToClient, this.closedClientsBytesToServer);
				}
				writeStickyFile();
			}
		}
	...
	public void logAmi() {
		...
		synchronized (this) {
			for (AmiWebBalancerClientSession i : this.clients.values()) {
				...
				ac.value = btc.value = bts.value = 0L;
				i.getConnectionStats(ac, btc, bts);
	...
	public byte[] getTestHttpMethod() {
		return this.testHttpMethod;
	}
