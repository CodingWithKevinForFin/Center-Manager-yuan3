...
public class AmiWebBalancerMain {
	...
	private static final String DEFAULT_TEST_URL = "/portal/rsc/ami/normal.png";
	private static final Logger log = LH.get();
	private static final String DEFAULT_TEST_PERIOD = "30 seconds";
	...
	public static void main2(ContainerBootstrap bs) {
		...
		try {
			...
			String testUrl = pc.getOptional(AmiWebBalancerProperties.PROPERTY_AMI_WEBBALANCER_SERVER_TEST_URL, DEFAULT_TEST_URL);
			...
			//			CachedFile sessionsFileCache = new CachedFile(sessionsFile, checkFilesFrequency);
			bs.getUseAggressiveThreadPool();
			ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(bs.getThreadPoolDefaultSizeProperty());
			AmiWebBalancerServer balancer = new AmiWebBalancerServer(executor, serversFileCache, sessionsFile, checkFilesFrequency, serverCheckPeriodMillis, sessionTimeout,
					testUrl, testPeriod);
			if (httpServerPort != null) {
				final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(ct, AmiWebBalancerProperties.PROPERTY_AMI_WEBBALANCER_HTTP_PORT_WHITELIST);
			...
			if (httpsServerPort != null) {
				final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(ct, AmiWebBalancerProperties.PROPERTY_AMI_WEBBALANCER_HTTPS_PORT_WHITELIST);
