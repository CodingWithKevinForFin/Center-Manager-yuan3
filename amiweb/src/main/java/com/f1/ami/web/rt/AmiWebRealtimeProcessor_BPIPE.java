...
public class AmiWebRealtimeProcessor_BPIPE extends AmiWebAbstractRealtimeProcessor {
	...
	public String getSessionID(boolean override) {
		return b.getSessionID(override);
	}
	...
	public void startSubscription(String ticker, String fields) {
		...
		Subscription subscription = new Subscription(ticker, fields, "", correlationID);
		...
		try {
			...
			if (b.getActiveSubcriptions().containsKey(correlationID)) {
				this.getService().getDesktop().getManager().showAlert("Already subscribed to " + ticker + ". Please unsubscribe first.");
			}
	...
	public void startSubscriptionLevel2(String ticker, String type) {
		...
		try {
			...
			if (b.getActiveSubcriptions().containsKey(correlationID)) {
				this.getService().getDesktop().getManager().showAlert("Already subscribed to " + ticker + "with type" + type + ". Please unsubscribe first.");
			}
			LH.severe(log, "SUBSCRIPTION FAILED", e);
		}
	}
	...
	public void unsubscribeAll() {
		...
		for (Entry<CorrelationID, AmiWebRealtimeProcessor_BPIPE> e : b.getActiveSubcriptions().entrySet()) {
			if (e.getValue() == this) {
				try {
