	...
	public boolean verifyForSubmit() {
		...
		if (OH.ne(userFieldValue, queryFieldValue) && queryField.getField().getValue() != null) {
			...
			if (isCurFieldChecked) {
				...
				FormPortletRadioButtonField newGroupCheckedField = (FormPortletRadioButtonField) CH.getOr(queryField.getField().getForm().getGroup2LastChecked(), mapKey, null);
				if (queryField.getField().getForm().getGroup2LastChecked().containsKey(mapKey) && newGroupCheckedField == null) {
					queryField.getField().getForm().getGroup2LastChecked().put(mapKey, queryField.getField());
				} else if (newGroupCheckedField != null)
