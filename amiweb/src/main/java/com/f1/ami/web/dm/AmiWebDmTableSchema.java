...
public class AmiWebDmTableSchema implements ToStringable, Lockable {
	...
	private void removeType(String name) {
		types.remove(name);
		classTypes.removeType(name);
	}

	public Table mapToSchema(Table source, boolean forceNew) {
		lock();
		return mapToSchema(source, source == null ? null : source.getRows(), forceNew);
	}
	...
	public Table mapToSchema(Table source, Collection<Row> sourceRows, boolean forceNew) {
		lock();
		if (source == null) {
			return newEmptyTable();
		}
		if (matches(source) && (sourceRows == source.getRows()))
			return forceNew ? new BasicTable(source) : source;
		ensureNamesClassesReady();
		Table target = new BasicTable(classes, names);
		if (source.getSize() == 0)
			return target;
	...
	private boolean matches(Table t) {
		...
		for (Column c : t.getColumns()) {
			String type = types.getNoThrow((String) c.getId());
			if (OH.ne(c.getTypeCaster(), classTypes.getType(type)))
				return false;
		}
		return true;
	}
