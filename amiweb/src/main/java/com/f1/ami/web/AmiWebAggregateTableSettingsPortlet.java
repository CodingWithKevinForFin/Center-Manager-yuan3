...
public class AmiWebAggregateTableSettingsPortlet extends AmiWebAbstractTableSettingsPortlet implements FormPortletContextMenuListener, FormPortletContextMenuFactory {
	...
	private FormPortlet formVarTypes;
	...
	private FormPortletMultiSelectField<String> varTypesField;
	//	private AmiCenterGetAmiSchemaResponse schema;
	private List<FormPortletSelectField<Byte>> typesOptions = new ArrayList<FormPortletSelectField<Byte>>();
	...
	protected void initForms() {
		...
		formVarTypes = new FormPortlet(generateConfig());
		...
		tabs.addChild("General settings", formA);
		tabs.addChild("Variable types", formVarTypes);
	...
	protected void submitChanges() {
		...
		this.groupVisibleOnlyToggle.setValue(target.isAggregateOnVisibleColumnsOnly());
		for (FormPortletSelectField<Byte> i : typesOptions)
			if (i.getValue() != AmiDatasourceColumn.TYPE_NONE)
				target.putUserDefinedVariable((String) i.getCorrelationData(), AmiUtils.getClassForValueType(i.getValue()));
	...
	private void init() {
		...
		formVarTypes.addField(new FormPortletTitleField(""));
		com.f1.base.CalcTypes vars = AmiWebUtils.getAvailableVariables(target.getService(), target);
		com.f1.base.CalcTypes usedVariables = target.getUsedVariables();
		for (String varname : usedVariables.getVarKeys()) {
			FormPortletSelectField<String> option = formVarTypes.addField(new FormPortletSelectField<String>(String.class, varname + ":"));
			Class<?> type = usedVariables.getType(varname);
			option.addOption(type.getName(), type.getSimpleName());
			option.setDisabled(true);
		}
		com.f1.base.CalcTypes def = target.getUserDefinedVariables();
		for (String varname : usedVariables.getVarKeys()) {
			if (usedVariables.getType(varname) != null)
				continue;
			Class<?> type = def.getType(varname);
			FormPortletSelectField<Byte> option = formVarTypes.addField(new FormPortletSelectField<Byte>(Byte.class, varname));
			option.setCorrelationData(varname);
			option.addOption(AmiDatasourceColumn.TYPE_NONE, "<auto>");
			Class<?> types = usedVariables.getType(varname);
			byte recommended = AmiUtils.getTypeForClass(types, AmiDatasourceColumn.TYPE_NONE);
			for (byte t : AmiUtils.TYPES) {
				option.addOption(t, AmiWebUtils.formatType(t) + (t == recommended ? " (recommended)" : ""));
			}
			if (type == null)
				option.setValue(AmiDatasourceColumn.TYPE_NONE);
			else
				option.setValue(AmiUtils.getTypeForClass(type, AmiDatasourceColumn.TYPE_NONE));
			typesOptions.add(option);
		}
		formVarTypes.addField(new FormPortletTitleField("(Variables that are in use can not be modified)").setStyle("_fg=#777777"));
	}
	@Override
