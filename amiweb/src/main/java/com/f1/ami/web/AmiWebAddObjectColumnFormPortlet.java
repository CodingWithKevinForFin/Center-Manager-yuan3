...
public class AmiWebAddObjectColumnFormPortlet extends GridPortlet
	...
	public AmiWebAddObjectColumnFormPortlet(PortletConfig config, AmiWebAbstractTablePortlet portlet, int columnPosition, AmiWebCustomColumn col) {
		...
		if (col != null) {
			...
			this.enableLastNDaysField.setValue(col.getEnableLastNDays());
	...
	public boolean onUpdate(boolean warnOnInvalidFormulas) {
		...
		Object disableFutureDays = this.disableFutureDatesField.getValue();
		...
		columnConfig.put(AmiWebCustomColumn.CONFIG_PRECISION, precision == null ? AmiConsts.DEFAULT : precision);
		...
		columnConfig.put(AmiWebCustomColumn.CONFIG_FIXED_WIDTH, isFixedWidth);
		columnConfig.put(AmiWebCustomColumn.CONFIG_TOOLTIP, tooltip);
		if (this.columnToCopy != null)
			columnConfig.put(AmiWebCustomColumn.CONFIG_EDIT_OPTIONS, this.columnToCopy.getEditSelectOptions());
		...
		try {
			...
			column = new AmiWebCustomColumn(this.amiPortlet, columnConfig, isAggregate);
