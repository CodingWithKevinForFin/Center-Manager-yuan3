...
public class AmiWebRealTimeTableSettingsPortlet extends AmiWebAbstractTableSettingsPortlet implements FormPortletContextMenuListener, FormPortletContextMenuFactory {
	...
	public void init() {
		...
		for (String name : usedVariables.getVarKeys()) {
			FormPortletSelectField<String> option = formVarTypes.addField(new FormPortletSelectField<String>(String.class, name + ":"));
			Class<?> type = usedVariables.getType(name);
		...
		for (String varname : vars.getVarKeys()) {
			...
			byte recommended = AmiUtils.getTypeForClass(vars.getType(varname), AmiDatasourceColumn.TYPE_NONE);
