...
public class AmiWebManageUsersPortlet extends GridPortlet implements WebContextMenuFactory, WebContextMenuListener, FormPortletListener, ConfirmDialogListener {
	...
	private String checkPlugins() {
		...
		if (entitlementMode.equals("off")) {
			String authPlugin = properties.getRequired(AmiWebProperties.PROPERTY_AMI_WEB_AUTH_PLUGIN_CLASS);
			String ssoPlugin = properties.getOptional(AmiWebProperties.PROPERTY_SSO_PLUGIN_CLASS, "none");
			ssoPlugin = ssoPlugin.equals("none") ? properties.getOptional(AmiWebProperties.PROPERTY_SAML_PLUGIN_CLASS, "none") : ssoPlugin;
			if (!authPlugin.equals("com.f1.ami.web.auth.AmiAuthenticatorFileBacked"))
	...
	private void onUpdate() {
		...
		for (Row r : this.fastTable.getTable().getRows()) {
			...
			if ("password".equals(encryptMode))
				newAccess.append("|")
						.append(properties.getServices().getService(AmiConsts.SERVICE_ENCRYPTER, AmiEncrypter.class).encrypt(passwordStore.get(r.get(COLUMN_USERNAME).toString())));
			else
				newAccess.append("|").append(passwordStore.get(r.get(COLUMN_USERNAME).toString()));
			newAccess.append("|").append("ISADMIN=" + r.get(COLUMN_ISADMIN).toString());
			newAccess.append("|").append("ISDEV=" + r.get(COLUMN_ISDEV).toString());
			if (r.get(COLUMN_DEFAULT_LAYOUT) != null && !SH.isEmpty(r.get(COLUMN_DEFAULT_LAYOUT).toString()))
				newAccess.append("|").append("DEFAULT_LAYOUT=" + r.get(COLUMN_DEFAULT_LAYOUT));
			if (r.get(COLUMN_LAYOUTS) != null && !SH.isEmpty(r.get(COLUMN_LAYOUTS).toString()))
				newAccess.append("|").append("LAYOUTS=" + r.get(COLUMN_LAYOUTS));
			if (r.get(COLUMN_PERMISSIONS) != null && !SH.isEmpty(r.get(COLUMN_PERMISSIONS).toString()))
				newAccess.append("|").append("AMIDB_PERMISSIONS=" + r.get(COLUMN_PERMISSIONS));
			if (r.get(COLUMN_OTHERS) != null && !SH.isEmpty(r.get(COLUMN_OTHERS).toString()))
	...
	public Row isUser(String username) {
		...
		for (Row r : t) {
			if (r.get(COLUMN_USERNAME).equals(username))
				return r;
		}
		return null;
	}
			...
			if (button == this.cxlButton)
				...
				if (verifyUser(username, password)) {
					if (this.isChangePassword) {
						...
						AmiWebManageUsersPortlet.this.putNoFire(row, COLUMN_ISADMIN, isAdminField.getValue().toString());
						AmiWebManageUsersPortlet.this.putNoFire(row, COLUMN_ISDEV, isDevField.getValue().toString());
