...
public class AmiWebManageHeadlessSessionsPortlet extends GridPortlet implements WebContextMenuFactory, WebContextMenuListener, FormPortletListener, ConfirmDialogListener {
	...
	private void onUpdate(String addedSession) {
		...
		for (Row r : this.fastTable.getTable().getRows()) {
			...
			newHeadless.append("|").append("ISADMIN=" + r.get(COLUMN_ISADMIN).toString());
			newHeadless.append("|").append("ISDEV=" + r.get(COLUMN_ISDEV).toString());
			...
			if (addedSession != null && addedSession.equals(r.get(COLUMN_SESSIONNAME).toString())) {
			...
			if ("DELETE".equals(source.getCallback())) {
				...
				for (Object row : selected) {
					if (row instanceof Row) {
						String sessionName = ((Row) row).get(COLUMN_SESSIONNAME).toString();
						AmiWebHeadlessSession session = headlessManager.getSessionByName(sessionName);
						if (session != null && session.getWebState() != null && session.getWebState().isAlive()) {
							hasActive = true;
							postMessage += ((Row) row).get(COLUMN_SESSIONNAME).toString() + ", ";
						} else {
							AmiWebHeadlessSession toRemove = headlessManager.getSessionByName(((Row) row).get(COLUMN_SESSIONNAME).toString());
							headlessManager.removeSession(toRemove);
				...
				String sessionName = source.getCorrelationData().toString();
	...
	public void onContextMenu(WebTable table, String action) {
		...
		if (action.equals(ACTION_COPY_SESSION))
			...
			String sessionName = selectedRow.get(COLUMN_SESSIONNAME).toString();
	...
	public class HeadlessSessionConfigPortlet extends GridPortlet implements FormPortletListener {
		...
		public HeadlessSessionConfigPortlet(PortletConfig config, Row user) {
			...
			if (user != null) {
				sessionResolutionField.setValue(user.get(COLUMN_RESOLUTION).toString());
