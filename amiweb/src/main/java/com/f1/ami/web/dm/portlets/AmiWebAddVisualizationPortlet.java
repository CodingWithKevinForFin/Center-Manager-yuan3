...
public class AmiWebAddVisualizationPortlet extends GridPortlet
	...
	public Portlet removePreviewPortlet() {
		...
		service.registerAmiUserPrefId(this.panelIdField.getValue(), t);
		return t;
	}
	...
	public void showChooseTableDialog(AmiWebDm dm) {
		...
		if (tableList.size() > 1) {
			...
			getManager()
					.showDialog("Choose Datasource Table", new ConfirmDialogPortlet(generateConfig(), "The chosen datamodel contains multiple tables. Please choose a table.",
							ConfirmDialogPortlet.TYPE_MESSAGE, this, selTableField).setCallback("CHOOSE_TABLE").updateButton(ConfirmDialogPortlet.ID_CLOSE, "Choose table"))
