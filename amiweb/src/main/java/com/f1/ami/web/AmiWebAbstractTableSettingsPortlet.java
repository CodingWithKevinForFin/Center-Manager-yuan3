...
public class AmiWebAbstractTableSettingsPortlet extends AmiWebPanelSettingsPortlet implements FormPortletContextMenuListener, FormPortletContextMenuFactory {
	...
	protected boolean verifyChanges() {
		com.f1.base.CalcTypes vars = AmiWebUtils.getAvailableVariables(getPortlet().getService(), getPortlet());
		...
		String bgFormulaString = this.rowBackgroundColorField.getValue();
		//		try {
		//			bgFormulaString = this.replaceTitlesWithVarsInFormula(this.rowBackgroundColorField.getValue());
		//		} catch (Exception e) {
		//			getManager().showAlert("Error with Row Background Color Formula: " + e.getMessage(), e);
		//			return false;
		//		}
		//		ToDoException.throwNow(); //		this.portlet.toFormula(bgFormulaString, errorSink, "Row Background Color", vars);
		if (errorSink.length() > 0) {
			getManager().showAlert(errorSink.toString());
			return false;
		}

		//		String txFormulaString;
		//		try {
		//			txFormulaString = this.replaceTitlesWithVarsInFormula(this.rowTextColorField.getValue());
		//		} catch (Exception e) {
		//			getManager().showAlert("Error with Row Text Color Formula: " + e.getMessage(), e);
		//			return false;
		//		}
		String txFormulaString = this.rowTextColorField.getValue();
		//		ToDoException.throwNow(); //		this.portlet.toFormula(txFormulaString, errorSink, "Row Text Color", vars);
	...
	protected void submitChanges() {
		...
		//			return;
		//		}
		//		DerivedCellCalculator bgFormula = this.portlet.toFormula(bgFormulaString, errorSink, "Row Background Color", vars);
		if (errorSink.length() > 0) {
			getManager().showAlert(errorSink.toString());
			return;
		}
		...
		//		DerivedCellCalculator txFormula = this.portlet.toFormula(txFormulaString, errorSink, "Row Text Color", vars);
