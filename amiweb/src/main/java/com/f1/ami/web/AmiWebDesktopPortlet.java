...
public class AmiWebDesktopPortlet extends GridPortlet implements WebDropDownMenuListener, WebDropDownMenuFactory, ConfirmDialogListener, ColorPickerListener,
	...
	private void loadDefaultLayout() {
		...
		if (this.desktop == null) {
			if (defaultLayoutSource != null) {
				...
				if (currentLayout != null && !AmiWebLayoutManager.DEFAULT_LAYOUT_NAME.equals(currentLayout)) {
	...
	public void showNewLayoutStylePortlet() {
		if (AmiWebLayoutManager.DEFAULT_LAYOUT_NAME.equals(this.service.getLayoutFilesManager().getLayoutName()) && !isLocked && desktop.getWindows().isEmpty())
	...
	public void onContextMenu(DropDownMenuPortlet portlet, String action) {
		...
		if ("manage_windows".equals(action)) {
			...
			getManager().getPendingJs()
					.append("window.open('" + BasicPortletManager.URL_START + "?" + AmiWebConsts.URL_PARAM_LAYOUT + "=&" + BasicPortletManager.KEEP_EXISTING_OPEN + "=true');");
			...
			service.getLayoutFilesManager().loadLayoutDialog(AmiWebLayoutManager.DEFAULT_LAYOUT_NAME, null, null);
		...
		} else if ("save_abs".equals(action)) {
			service.getLayoutFilesManager().saveLayout();
		} else if ("save_cloud".equals(action)) {
			service.getLayoutFilesManager().saveLayout();
	...
	public void logout() {
		BPIPEPlugin b = this.getService().getBPIPE();
		if (b != null)
			b.onLogout();
		...
		if ("file".equals(id)) {
			...
			if (this.varsManager.isUserDev() || this.varsManager.isUserAdmin()) {
				...
				if (!isLocked) {
					...
					String source = this.service.getLayoutFilesManager().getLayoutSource();
					boolean enabled = !AmiWebLayoutManager.DEFAULT_LAYOUT_NAME.equals(service.getLayoutFilesManager().getLayoutName());
					boolean hasInfo = true;
					if (enabled) {
						if (AmiWebConsts.LAYOUT_SOURCE_LOCAL.equals(source) || source == null)
							r.addChild(new BasicWebMenuLink("Save", true, "save").setBackgroundImage(AmiWebConsts.ICON_SAVE));
						else if (AmiWebConsts.LAYOUT_SOURCE_ABSOLUTE.equals(source) || source == null)
							r.addChild(new BasicWebMenuLink("Save", true, "save_abs").setBackgroundImage(AmiWebConsts.ICON_SAVE));
						else if (AmiWebConsts.LAYOUT_SOURCE_CLOUD.equals(source) || source == null)
							r.addChild(new BasicWebMenuLink("Save", true, "save_cloud").setBackgroundImage(AmiWebConsts.ICON_SAVE));
						else {
							hasInfo = false;
							r.addChild(new BasicWebMenuLink("Save", false, "").setBackgroundImage(AmiWebConsts.ICON_SAVE));
						}
					}
					r.addChild(k);
					r.addChild(w);
					...
					r.addChild(new BasicWebMenuLink("View Changes...", true, "diff_saved").setBackgroundImage(AmiWebConsts.ICON_DIFF));
	...
	public void onPortletClosed(Portlet newPortlet) {
		...
		if (newPortlet instanceof AmiWebPortlet) {
			...
			if (!hidden && newPortlet instanceof AmiWebPortlet) {
				...
				if (!fromLinks.isEmpty() || !toLinks.isEmpty()) {
					List<AmiWebDmLink> toRemove = new ArrayList<AmiWebDmLink>(fromLinks.size() + toLinks.size());
					toRemove.addAll(fromLinks);
					toRemove.addAll(toLinks);
					for (AmiWebDmLink i : toRemove)
	...
	public void onMenuItem(String id) {
		getService().getSecurityModel().assertPermitted(this, id, "show_window_*");
