...
public class AmiWebMainHelper {
	...
	public static void initHttpServerSockets(ContainerTools tools, PropertyController props, BasicHttpServer httpServer) throws IOException {
		...
		if (httpPort != null) {
			final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(tools, AmiWebProperties.PROPERTY_HTTP_PORT_WHITELIST);
			for (String s : SH.split(',', httpPort))
				httpServer.addServerSocket(new HttpServerSocket(httpPortBindAddr, sse, SH.parseInt(s)));
		}
		if (httpsPort != null) {
			final HttpServerSocket ssocket = AmiWebMainHelper.createHttpsServerSocket(tools, props, httpsPortBindAddr, httpsPort);
	...
	private static HttpServerSocket createHttpsServerSocket(ContainerTools tools, PropertyController props, String httpsPortBindAddr, Integer httpsPort) {
		final ServerSocketEntitlements sse = AmiUtils.parseWhiteList(tools, AmiWebProperties.PROPERTY_HTTPS_PORT_WHITELIST);
