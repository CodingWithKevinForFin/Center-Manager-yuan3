...
public class AmiWebConsole {
	...
	public String showPanels(String sessionId) {
		...
		try {
			...
			t.getRows().addRow("Desktop - " + rootPortlet.getTitle(), "Main Window", null, rootPortlet.getVisible(), rootPortlet.getWidth(), rootPortlet.getHeight(), null);
			...
			for (Window window : desktop2.getWindows())
				if (window.isPoppedOut())
					addPanels(t, "Browser Window - " + window.getName(), 0, window.getPortlet());
			return TableHelper.toString(t, "", TableHelper.SHOW_ALL_BUT_TYPES);
		} finally {
