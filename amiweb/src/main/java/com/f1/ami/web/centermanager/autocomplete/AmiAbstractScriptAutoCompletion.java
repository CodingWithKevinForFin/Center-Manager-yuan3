...
import com.f1.ami.web.AmiWebDocumentationPortlet;
import com.f1.ami.web.AmiWebMenuUtils;
import com.f1.ami.web.AmiWebMethodPortlet;
...
public abstract class AmiAbstractScriptAutoCompletion implements WebMenuListener {
	...
	public AmiAbstractScriptAutoCompletion(AmiWebService service) {
	...
	public AmiAbstractScriptAutoCompletion reset(AmiAbstractFormPortletAmiScriptField field, int cp) {
		...
		if (fullCommandEnd != -1) {
			...
			try {
				DerivedCellCalculator calc = toCalcNotOptimized();
		...
		if (this.funcStart != -1 && this.cursorPosition == this.cmdStart) {
			...
			if (func.indexOf('.') != -1) {
				...
				//Hint on static Function contents //very important
				List<MethodFactory> sink = new ArrayList<MethodFactory>();
				getMethodFactory().getAllMethodFactories(sink);
		...
		if (cmd.indexOf('.') != -1) {
			...
			if (funcStart == -1) {
				...
				List<MenuOption> sink = new ArrayList<AmiAbstractScriptAutoCompletion.MenuOption>();
	...
	private static final Comparator<MenuOption> MENU_SORTER = new Comparator<AmiAbstractScriptAutoCompletion.MenuOption>() {
	...
	public void onMenuItem(String id) {
		...
		if (option.type == MenuOption.TYPE_HINT) {
			...
			//post onAutoComplete
			if (option.type == MenuOption.TYPE_FUNC) {
	...
	//get method factory from its scriptmanager
	public abstract BasicMethodFactory getMethodFactory();

	protected abstract Class<?> evaluateType(String prefix);

	public abstract DerivedCellCalculator toCalcNotOptimized();
}
