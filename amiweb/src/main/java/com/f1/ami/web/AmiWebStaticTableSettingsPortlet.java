...
class AmiWebStaticTableSettingsPortlet extends AmiWebAbstractTableSettingsPortlet implements FormPortletContextMenuListener, FormPortletContextMenuFactory, ChooseDmListener {
	...
	private FormPortlet varTypesFormPortlet;
	...
	protected void initForms() {
		...
		this.varTypesFormPortlet = new FormPortlet(generateConfig());
		this.buttonsFormPortlet = new FormPortlet(generateConfig());
		this.tabPortlet = new TabPortlet(generateConfig());
		this.tabPortlet.addChild("General Settings", this.settingsFormPortlet);
		this.tabPortlet.addChild("Variable Types", this.varTypesFormPortlet);
		...
		this.addChild(this.tabPortlet, 0, 0);
		...
		for (String i : CH.sort(usedVariables.getVarKeys(), SH.COMPARATOR_CASEINSENSITIVE_STRING)) {
			FormPortletSelectField<String> option = varTypesFormPortlet.addField(new FormPortletSelectField<String>(String.class, i + ":"));
			Class<?> type = usedVariables.getType(i);
			option.addOption(type.getName(), type.getSimpleName());
			option.setDisabled(true);
		}
		varTypesFormPortlet.addField(new FormPortletTitleField("(Variable types can not be modified)").setStyle("_fg=#777777"));
	}
