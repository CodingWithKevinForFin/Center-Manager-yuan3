...
OpenFinServer.prototype.wait =false;
...
OpenFinServer.prototype.addListener=function(channel){
	...
	var that = this;
	fin.InterApplicationBus.subscribe({uuid: "*"}, channel, (sub_msg, sub_uuid, sub_name) => that.peer.sendToServer('onMessage', channel, sub_msg)).catch(error => that.peer.sendToServer('onError', 'addListener', 'for channel '+channel+': ' +JSON.stringify(error)));
//	fin.InterApplicationBus.subscribe({uuid: "*"}, channel, 
//			function(sub_msg, sub_uuid, sub_name){
//				that.peer.sendToServer('onMessage',channel,sub_msg);
//			}
//	);
//	}
